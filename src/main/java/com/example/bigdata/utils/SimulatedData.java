...
public class SimulatedData {
    public static void main(String[] args) {
        File file = new File("C:\\Users\\cloud\\Desktop\\simulate_data.txt");

        try (FileWriter fw = new FileWriter(file);
            PrintWriter pw = new PrintWriter(fw);
        ) {
            String[] citys =  { "北京市","天津市","石家庄市","唐山市","秦皇岛市","邯郸市","邢台市","保定市","张家口市","承德市","衡水市","廊坊市","沧州市","太原市","大同市","阳泉市","长治市","晋城市","朔州市","晋中市","运城市","忻州市",
            ...
            String[] pollutions = {"细颗粒物(PM2.5)", "颗粒物(PM10)", "-"};
            String[] qualitys = {"轻度污染", "中度污染", "重度污染" ,"严重污染","优","良"};
/*            for(int i = 1; i <= 5000000; i++) {
                int randomNum = (int)(Math.random()*1000);
                int randomNum2 = (int)(Math.random()*1000);
                int randomNum3 = (int)(Math.random()*1000);

                String str = i + " " + randomNum + " " + randomNum2 + " " + randomNum3 + " simulate_data";
                pw.println(str);
            }*/
            for(int i = 1; i <= 500000; i++) {
                String city = citys[(int) (Math.random() * 408)];
                String pollution = pollutions[(int) (Math.random() * 3)];
                String quality = qualitys[(int) (Math.random() * 6)];
                String O3 = String.valueOf((int) (Math.random() * 100));
                String PM2_5 = String.valueOf((int) (Math.random() * 100));
                String CO = String.valueOf((int) (Math.random() * 100));
                String NO2 = String.valueOf((int) (Math.random() * 100));
                String SO2 = String.valueOf((int) (Math.random() * 100));
                String AQI = String.valueOf((int) (Math.random() * 100));
                String PM10 = String.valueOf((int) (Math.random() * 100));

                String line = String.format("%s %s %s %s %s %s %s %s %s %s", city
                        , pollution, quality, O3, PM2_5, CO, NO2, SO2, AQI, PM10);
